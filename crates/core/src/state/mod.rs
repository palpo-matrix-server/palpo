...
mod tests {
    ...
    fn test_event_sort() {
        ...
        let sorted_power_events =
            crate::reverse_topological_power_sort(power_events, &auth_chain, |id| events.get(id).map(Arc::clone))
                .unwrap();
        ...
        let sorted_event_ids =
            crate::mainline_sort(&events_to_sort, power_level, |id| events.get(id).map(Arc::clone)).unwrap();
