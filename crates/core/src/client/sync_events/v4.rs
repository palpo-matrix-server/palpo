...
use crate::{
    DeviceKeyAlgorithm, OwnedMxcUri, OwnedRoomId, OwnedUserId, RoomId, UnixMillis,
    device::DeviceLists,
    directory::RoomTypeFilter,
    events::{
        AnyGlobalAccountDataEvent, AnyRoomAccountDataEvent, AnyStrippedStateEvent, AnySyncStateEvent,
        AnySyncTimelineEvent, AnyToDeviceEvent, StateEventType, TimelineEventType, receipt::SyncReceiptEvent,
...
pub struct SyncEventsReqBody {
    ...
    ///
    /// Please consult ["Bandwidth optimisations for persistent clients" of the
    /// MSC][MSC] for further details, expectations of the implementation
    /// and limitations to consider before implementing this.
    ///
...
pub struct SyncEventsResBody {
    ...
    /// token has been discard by the server?).
...
pub struct ReqListFilters {
    ...
    ///
    /// Flag which only returns rooms present (or not) in the DM section of
    /// account data. If unset, both DM rooms and non-DM rooms are returned.
    ...
    ///
    /// A list of spaces which target rooms must be a part of. For every
    /// invited/joined room for this user, ensure that there is a parent
    /// space event which is in this list. If unset, all rooms are included.
    /// Servers MUST NOT navigate subspaces. It is up to the client to give
    /// a complete list of spaces to navigate. Only rooms directly in these
    ...
    ///
    /// Flag which only returns rooms which have an `m.room.encryption` state
    /// event. If unset, both encrypted and unencrypted rooms are returned.
    /// If false, only unencrypted rooms are returned. If true, only
    /// encrypted rooms are returned.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Whether to return Rooms with tombstones, only rooms without tombstones
    /// or both.
    ///
    /// Flag which only returns rooms which have an `m.room.tombstone` state
    /// event. If unset, both tombstoned and un-tombstoned rooms are
    /// returned. If false, only un-tombstoned rooms are returned. If true,
    /// only tombstoned rooms are returned.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub is_tombstoned: Option<bool>,

    /// Only list rooms of given create-types or all.
    ///
    /// If specified, only rooms where the `m.room.create` event has a `type`
    /// matching one of the strings in this array will be returned. If this
    /// field is unset, all rooms are returned regardless of type. This can
    /// be used to get the initial set of spaces for an account.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    ...
    ///
    /// Filter the room name. Case-insensitive partial matching e.g 'foo'
    /// matches 'abFooab'. The term 'like' is inspired by SQL 'LIKE', and
    /// the text here is similar to '%foo%'.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// with tags A and B with filters `tags:[A]` `not_tags:[B]` would NOT be
    /// included because `not_tags` takes priority over `tags`. This filter
    /// is useful if your Rooms list does NOT include the list of favourite
    /// rooms again.
    #[serde(default, skip_serializing_if = "<[_]>::is_empty")]
...
pub struct ReqList {
    ...
    /// that might be, the clients wants all rooms the filters apply to. When
    /// operating in this mode, `ranges` and  `sort` will be ignored  there
    /// will be no movement operations (`DELETE` followed by `INSERT`) as
    /// the client has the entire list and can work out whatever sort order
    /// they wish. There will still be `DELETE` and `INSERT` operations when
    /// rooms are left or joined respectively. In addition, there will be an
    /// initial `SYNC` operation to let the client know which rooms in the
    /// rooms object were from this list.
    #[serde(default, skip_serializing_if = "crate::serde::is_default")]
    ...
    /// If tombstoned rooms should be returned and if so, with what information
    /// attached
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// An allow-list of event types which should be considered recent activity
    /// when sorting `by_recency`. By omitting event types from this field,
    /// clients can ensure that uninteresting events (e.g. a profil rename)
    /// do not cause a room to jump to the top of its list(s). Empty or
    /// omitted `bump_event_types` have no effect; all events in a room will be
    /// considered recent activity.
    ///
    /// NB. Changes to bump_event_types will NOT cause the room list to be
    /// reordered; it will only affect the ordering of rooms due to future
    /// updates.
    ///
    /// Sticky.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub struct SyncRoom {
    ...
    /// If this is `Some(_)`, this is a not-yet-accepted invite containing the
    /// given stripped state events.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// The prev_batch allowing you to paginate through the messages before the
    /// given ones.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// The number of users with membership of `join`, including the client’s
    /// own user ID.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    ///
    /// It's not to be confused with `origin_server_ts` of the latest event in
    /// the timeline. `bump_event_types` might "ignore” some events when
    /// computing the timestamp of the room. Thus, using this `timestamp`
    /// value is more accurate than relying on the latest event.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
//         assert_eq!(serde_json::to_string(&entry).unwrap().as_str(),
// r#""*""#);
...
//         assert_eq!(
//
// serde_json::from_str::<RoomReceiptConfig>(r#""!n8f893n9:example.com""#).
// unwrap(),
// RoomReceiptConfig::Room(owned_room_id!("!n8f893n9:example.com"))         );
//     }
// }
