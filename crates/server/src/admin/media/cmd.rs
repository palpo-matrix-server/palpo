...
use crate::{
	AppError, AppResult, debug, debug_info, debug_warn, error, info, trace,
...
pub(super) async fn delete(
	...
	if event_id.is_some() && mxc.is_some() {
		return Err(AppError::public("Please specify either an MXC or an event ID, not both."));
	}
	...
	if let Some(mxc) = mxc {
		...
		return Err(AppError::public("Deleted the MXC from our database and on our filesystem."));
	}
