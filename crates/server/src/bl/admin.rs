...
enum AdminCommand {
    ...
    /// Retrieve and print a PDU by ID from the Palpus database
    GetPdu {
    ...
    crate::room::timeline::build_and_append_pdu(
            PduBuilder {
                ...
                content: to_raw_value(&RoomMessageEventContent::text_html(
                        format!("## Thank you for trying out Palpus!\n\nPalpus is currently in Beta. This means you can join and participate in most Matrix rooms, but not all features are supported and you might run into bugs from time to time.\n\nHelpful links:\n> Website: https://palpo.im\n> Git and Documentation: https://gitlab.com/famedly/palpo\n> Report issues: https://gitlab.com/famedly/palpo/-/issues\n\nFor a list of available commands, send the following message in this room: `@palpo:{}: --help`\n\nHere are some rooms you can join (by typing the command):\n\nPalpus room (Ask questions and get notified on updates):\n`/join #palpo:fachschaften.org`\n\nPalpus lounge (Off-topic, only Palpus users are allowed to join)\n`/join #palpo-lounge:palpo.im`", &conf.server_name),
                        format!("<h2>Thank you for trying out Palpus!</h2>\n<p>Palpus is currently in Beta. This means you can join and participate in most Matrix rooms, but not all features are supported and you might run into bugs from time to time.</p>\n<p>Helpful links:</p>\n<blockquote>\n<p>Website: https://palpo.im<br>Git and Documentation: https://gitlab.com/famedly/palpo<br>Report issues: https://gitlab.com/famedly/palpo/-/issues</p>\n</blockquote>\n<p>For a list of available commands, send the following message in this room: <code>@palpo:{}: --help</code></p>\n<p>Here are some rooms you can join (by typing the command):</p>\n<p>Palpus room (Ask questions and get notified on updates):<br><code>/join #palpo:fachschaften.org</code></p>\n<p>Palpus lounge (Off-topic, only Palpus users are allowed to join)<br><code>/join #palpo-lounge:palpo.im</code></p>\n", &conf.server_name),
                ))
                .expect("event is valid, we just created it"),
                unsigned: None,
                state_key: None,
