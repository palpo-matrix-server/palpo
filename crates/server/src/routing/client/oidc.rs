...
fn get_provider_config(provider_name: &str) -> Result<&'static OidcProviderConfig, MatrixError> {
    let config = config::get();
    let oidc_config = config.enabled_oidc()
        .ok_or_else(|| MatrixError::not_found("OIDC not enabled"))?;
    
    oidc_config.providers.get(provider_name)
...
async fn discover_provider_endpoints(provider_config: &OidcProviderConfig) -> Result<OidcProviderInfo, MatrixError> {
    let discovery_url = format!("{}/.well-known/openid-configuration", provider_config.issuer);
    ...
    match response {
        Ok(resp) if resp.status().is_success() => {
            ...
            let discovery: serde_json::Value = resp.json().await
                .map_err(|e| MatrixError::unknown(format!("Failed to parse discovery document: {}", e)))?;
            
            Ok(OidcProviderInfo {
                authorization_endpoint: discovery["authorization_endpoint"]
                    .as_str()
                    .ok_or_else(|| MatrixError::unknown("Missing authorization_endpoint in discovery"))?
                    .to_string(),
...
pub async fn oidc_auth(req: &mut Request, res: &mut Response) -> AppResult<()> {
    ...
    let provider_name = req.query::<String>("provider")
        .or_else(|| oidc_config.default_provider.clone())
        .ok_or_else(|| MatrixError::invalid_param("No OIDC provider specified and no default configured"))?;
    
    let provider_config = oidc_config.providers.get(&provider_name)
        .ok_or_else(|| MatrixError::not_found(format!("Unknown OIDC provider: {}", provider_name)))?;
...
pub async fn oidc_callback(req: &mut Request) -> JsonResult<OidcLoginResponse> {
    ...
    if let Some(error) = req.query::<String>("error") {
        let error_description = req.query::<String>("error_description")
            .unwrap_or_else(|| "No description provided".to_string());
        
        tracing::warn!("OIDC provider returned error: {} - {}", error, error_description);
    ...
    let session_cookie = req.cookie("oidc_session")
    ...
    let provider_config = oidc_config.providers.get(&session.provider)
        .ok_or_else(|| MatrixError::unknown(format!("Provider '{}' no longer configured", session.provider)))?;
    ...
    let matrix_user_id = generate_matrix_user_id(&user_info, oidc_config, config.server_name.as_str())?;
...
async fn exchange_code_for_tokens(
    ...
    if !response.status().is_success() {
        ...
        return Err(MatrixError::unknown(format!("Token exchange failed: HTTP {}", status)));
    }
...
async fn get_user_info_from_provider(
    ...
    if !response.status().is_success() {
        let status = response.status();
        let error_text = response.text().await.unwrap_or_default();
        tracing::error!("User info request failed with status {}: {}", status, error_text);
        return Err(MatrixError::unknown(format!("User info request failed: HTTP {}", status)));
    }
...
fn validate_user_info(user_info: &OidcUserInfo, oidc_config: &crate::config::OidcConfig) -> Result<(), MatrixError> {
    // Check email verification requirement
    if oidc_config.require_email_verified {
        if user_info.email.is_none() {
            return Err(MatrixError::forbidden("Email address is required for authentication", None));
        }
        
        if user_info.email_verified != Some(true) {
            return Err(MatrixError::forbidden("Email address must be verified with the identity provider", None));
        }
    }
...
fn generate_matrix_user_id(
    ...
    if sanitized.is_empty() {
        return Err(MatrixError::invalid_param("Cannot generate valid Matrix user ID from OIDC identity"));
    }
...
fn generate_display_name(user_info: &OidcUserInfo, provider_config: &OidcProviderConfig) -> String {
    ...
    user_info.name.clone()
        ...
        .unwrap_or_else(|| format!("User {}", &user_info.sub[..std::cmp::min(8, user_info.sub.len())]))
}
...
async fn create_or_get_user(
    ...
    if !oidc_config.allow_registration {
        return Err(MatrixError::forbidden("New user registration via OIDC is disabled", None));
    }
