...
pub async fn get_power_levels(room_id: &RoomId) -> AppResult<RoomPowerLevels> {
    ...
    println!("xxxxxxxxxxxxxxxxxxxx==============power levels content: {content:#?}");
    let power_levels = RoomPowerLevels::new(
        content.into(),
        &rules.authorization,
        creators,
    );
    println!("xxxxxxxxxxxxxxxx==============power levels: {power_levels:#?}");
    Ok(power_levels)
}
...
pub fn room_rules(room_version: &RoomVersionId) -> AppResult<RoomVersionRules> {
