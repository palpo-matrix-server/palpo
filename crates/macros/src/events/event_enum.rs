...
///
/// DO NOT alter the field names unless the structs in `palpo_core::events::event_kinds` have
/// changed.
...
pub fn expand_event_enums(input: &EventEnumDecl) -> syn::Result<TokenStream> {
    ...
    if matches!(kind, EventKind::State) {
        ...
        res.extend(
            expand_event_enum(kind, V::Initial, events, docs, attrs, variants, palpo_core)
                .unwrap_or_else(syn::Error::into_compile_error),
        );
    }
...
fn expand_into_full_event(
    ...
    Ok(quote! {
        ...
        impl #ident {
            ...
            pub fn into_full_event(self, room_id: crate::OwnedRoomId) -> #full {
                match self {
                    #(
                        #self_variants(event) => {
...
fn expand_content_enum(
    ...
    let content: Vec<_> = events
        .iter()
        .map(|event| event.to_event_content_path(kind, None))
        .collect();
    ...
    Ok(quote! {
        ...
        impl #palpo_core::events::EventContent for #ident {
            type EventType = #palpo_core::events::#event_type_enum;
...
fn field_return_type(name: &str, _palpo_core: &TokenStream) -> TokenStream {
    match name {
        "origin_server_ts" => quote! { crate::UnixMillis },
        "room_id" => quote! { &crate::RoomId },
        "event_id" => quote! { &crate::EventId },
        "sender" => quote! { &crate::UserId },
        _ => panic!("the `palpo_macros::event_enum::EVENT_FIELD` const was changed"),
    }
}
...
impl EventEnumEntry {
    ...
    pub(crate) fn stable_name(&self) -> syn::Result<&LitStr> {
        if self.ev_type.value().starts_with("m.") {
            ...
            self.aliases
                .iter()
                .find(|alias| alias.value().starts_with("m."))
                .ok_or_else(|| {
                    syn::Error::new(
                        Span::call_site(),
                        format!(
