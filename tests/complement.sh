...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestFederationKeyUploadQuery/Can_claim_remote_one_time_key_using_POST" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
