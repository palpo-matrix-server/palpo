...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestPowerLevels/PUT_power_levels_should_not_explode_if_the_old_power_levels_were_empty" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
