...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 1h -run "TestDeviceListUpdates/when_joining_a_room_with_a_local_user" -json ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
