...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestRoomsInvite/Parallel/Invited_user_can_reject_invite_for_empty_room" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
