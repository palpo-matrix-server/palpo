...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 1h -run "TestToDeviceMessagesOverFederation/good_connectivity" -json ./tests/ ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
