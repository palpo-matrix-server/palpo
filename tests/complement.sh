...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestLeftRoomFixture/Can_get_rooms/{roomId}/state_for_a_departed_room" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
