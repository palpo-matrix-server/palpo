...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestRoomCreationReportsEventsToMyself/parallel/Joining_room_twice_is_idempotent" -json ./tests/ ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
