...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestRemotePresence/Presence_changes_are_also_reported_to_remote_room_members" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
