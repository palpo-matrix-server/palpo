...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 1h -run "TestDeviceListUpdates/when_remote_user_joins_a_room" -json ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
