...
env \
    -C "$COMPLEMENT_SRC" \
    COMPLEMENT_BASE_IMAGE="$TEST_IMAGE" \
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestArchivedRoomsHistory/timeline_has_events/initial_sync" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
