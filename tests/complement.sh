...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestUserAppearsInChangedDeviceListOnJoinOverFederation" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
