...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestEventAuth/returns_the_auth_chain_for_the_requested_event" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
