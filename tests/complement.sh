...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "^(TestKnocking/A_user_in_the_room_can_reject_a_knock)$" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
