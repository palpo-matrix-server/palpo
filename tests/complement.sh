...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestRoomCanonicalAlias/Parallel/m.room.canonical_alias_rejects_invalid_aliase" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
