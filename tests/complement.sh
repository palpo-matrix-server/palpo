...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestMediaWithoutFileName/parallel/Can_download_without_a_file_name_over_federation" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
