...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 1h -run "TestFederationRoomsInvite/Parallel/Invited_user_can_reject_invite_over_federation" -json ./tests/ ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
