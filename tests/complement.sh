...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestArchivedRoomsHistory/timeline_is_empty/initial_sync" -json ./tests/ ./tests/csapi| tee "$LOG_FILE.jsonl"
set -o pipefail
