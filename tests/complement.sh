...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestRequestEncodingFails/POST_rejects_invalid_utf-8_in_JSON" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
