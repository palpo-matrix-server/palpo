...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -run "TestDeviceListUpdates/when_local_user_leaves_a_room" -json "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
