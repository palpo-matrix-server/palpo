...
env \
    ...
    go test -tags="palpo_blacklist" "$SKIPPED_COMPLEMENT_TESTS" -timeout 2h -json -run "TestDeviceManagement/DELETE_/device/{deviceId}" "${test_packages[@]}"| tee "$LOG_FILE.jsonl"
set -o pipefail
